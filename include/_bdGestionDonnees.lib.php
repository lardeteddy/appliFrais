...
function obtenirDetailVisiteur($idCnx, $unId) {
    ...
    $requete = "select id, nom, prenom from visiteur where id='" . $id . "'";
...
/* Meme fonction pour les comptable */

function obtenirDetailComptable($idCnx, $unId) {
    $id = filtrerChainePourBD($unId);
    $requete2 = "select id, nom, prenom from comptable where id='" . $id . "'";
    $idJeuRes = mysql_query($requete2, $idCnx);
    $ligne = false;
    if ($idJeuRes) {
        $ligne = mysql_fetch_assoc($idJeuRes);
        mysql_free_result($idJeuRes);
...
/**
 * Modifie une ligne hors forfait.
 * Modifie dans la BD la ligne hors forfait d'id $unIdLigneHF
 * @param resource $idCnx identifiant de connexion
 * @param string $idLigneHF id de la ligne hors forfait
 * @param string $dateHF date du frais hors forfait
 * @param string $libelleHF libelle du frais hors forfait
 * @param int $montantHF montant du frais hors forfait
 * @return 
 */
function ModifierLigneHF($idCnx, $unIdLigneHF, $dateHF, $libelleHF, $montantHF) {
    $requeteModif = "update lignefraishorsforfait set date='" . $dateHF . "' , libelle='" . $libelleHF . "' ,  montant= " . $montantHF . " where id =" . $unIdLigneHF;
    mysql_query($requeteModif, $idCnx);
}
...
function verifierInfosConnexionComptable($idCnx, $unLogin, $unMdp) {
    ...
    $unMdp = filtrerChainePourBD($unMdp);
    // le mot de passe est crypt� dans la base avec la fonction de hachage md5
    $req2 = "select id, nom, prenom, login, mdp from comptable where login='" . $unLogin . "' and mdp='" . $unMdp . "'";
    $idJeuRes = mysql_query($req2, $idCnx);
    $ligne = false;
    if ($idJeuRes) {
        $ligne = mysql_fetch_assoc($idJeuRes);
        mysql_free_result($idJeuRes);
 ...
 * @param resource $idCnx identifiant de connexion
 * @param string $unIdVisiteur visiteur selectionné
 * @param string $unMois mois sous la forme aaaamm
 * @param string $unEtat l'etat a modifier
 * @return void 
 */
function modifierEtatFicheFrais($idCnx, $unMois, $unIdVisiteur, $unEtat, $nbJustificatifs) {
    $requete = "update fichefrais set idEtat = '" . $unEtat .
            "', dateModif = now(), nbJustificatifs='". $nbJustificatifs ."' where idVisiteur ='" .
            $unIdVisiteur . "' and mois = '" . $unMois . "'";
    mysql_query($requete, $idCnx);
}

/**
 * Recupere les données des fiches forfaitisées et hors forfait
 * 
 * @param type $idCnx identifiant de connexion
 */
function obtenirDonneesFicheFrais() {
    $requete = "Select * From fichefrais where idEtat='VA'";
    return $requete;
}
 ...
 * @param type $mois
 */
function calculerMontantTotal($idVisiteur, $mois) {

    //requere permettant de recupérer les données de la ligne forfaitisées du visiteur et du mois indiqués en paramètre
    $RequeteRecupInfo = mysql_query("SELECT idFraisForfait, quantite from lignefraisforfait 
        where idVisiteur='" . $idVisiteur . "' and mois='" . $mois . "' ");
    while ($lgeltsforfait = mysql_fetch_array($RequeteRecupInfo)) {
    ...
    mysql_free_result($RequeteRecupInfo);

    //Requete permettant de recupérer les coefficient de reboursement
    ...
    //Fin de la requete permettant de récupérer les coefficient de remboursement
    
    //Debut du calcul des montants des lignes hors forfait non refusé
    $RequeteHForfait = "Select montant from lignefraishorsforfait where idVisiteur='" . $idVisiteur . "' and mois='" . $mois . "' and libelle not like '[Refus]%'";
    $idJeuFraisForfaitHF = mysql_query($RequeteHForfait);
    ...
    //Fin du calcul des montants des lignes hors forfait non refusé
    
    $Calcul = ($km * $kmBase) + ($etp * $etpBase) + ($nui * $nuiBase) + ($rep * $repBase) + $CalculHF;
    
    $requeteInsert = "UPDATE fichefrais SET montantValide = '" . $Calcul . "' where idVisiteur='" . $idVisiteur . "' and mois='" . $mois . "' ";
